...
if __name__ == "__main__":
    ...
    def packet_processor(packet):
        feature_dict = nsl_kdd_packet_parser(packet)
        if feature_dict:
            print(f"Processed Packet. Current Count in window: {feature_dict["count"]}")
            print(feature_dict)

    # Start sniffing on your primary interface
    sniff(filter="ip", prn=packet_processor, store=0)
